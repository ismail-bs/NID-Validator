...
export class ValidationPipe implements PipeTransform<any> {
  async transform(value: any, { metatype }: ArgumentMetadata) {
    ...
    const errorsResponse = { message: '', errors: [] };
      ...
      errors.map((err: ValidationError) => {
        ...
        err &&
          ...
          Object.values(err.constraints).forEach((constraint) => {
            errorsResponse.message += (constraint as string) + ',';
        ...
        err.property &&
          errorArray &&
          errorArray.length &&
          errorsResponse.errors.push(...errorArray);
      });
      if (errorsResponse?.message || errorsResponse?.errors?.length) {
